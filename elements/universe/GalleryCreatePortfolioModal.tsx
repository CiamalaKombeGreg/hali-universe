...
export default function GalleryCreatePortfolioModal({
  open,
  onClose,
  onSaved,
}: Props) {
  ...
  const hasContent = useMemo(
    () => name.trim().length > 0 && images.length > 0,
    [name, images.length]
  );
  ...
  async function handleSave() {
    ...
    try {
      const finalImages: CreateGalleryPortfolioPayload["images"] = [];
      ...
      const payload: CreateGalleryPortfolioPayload = {
        name: name.trim(),
        images: finalImages,
      };

      const response = await fetch("/api/gallery/portfolios", {
        method: "POST",
        headers: {
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 px-6 py-6 backdrop-blur-md">
      <div className="max-h-[92vh] w-full max-w-[1400px] overflow-hidden rounded-[30px] border border-white/10 bg-[#07111f]/95 shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-5">
          <div>
            <h2 className="text-2xl font-black text-white">Create Portfolio</h2>
            <p className="mt-1 text-sm text-white/55">
              Add uploaded or external images to a standalone portfolio.
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-[24px] border border-white/10 bg-black/20 p-5">
              ...
              <button
                ...
                className="flex min-h-[180px] w-full flex-col items-center justify-center rounded-[24px] border border-dashed border-cyan-300/25 bg-cyan-400/5 p-6 text-center transition hover:border-cyan-300/45 hover:bg-cyan-400/10"
              >
                ...
                <p className="mt-2 max-w-md text-sm leading-6 text-white/60">
                  Choose local images to add into the portfolio. They upload only when
                  you save.
                </p>
              </button>
            </section>
                  ...
                  images.map((image) => (
                    ...
                    >
                      ...
                      <div className="overflow-hidden rounded-[18px] border border-white/10 bg-black/20">
                        ...
                        <img
                          src={image.imageUrl}
                          alt={image.altText || image.title || "Pending gallery image"}
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateImage(image.id, { externalLink: e.target.value })
                          }
                          className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none"
            ...
            <section className="rounded-[24px] border border-white/10 bg-black/20 p-5">
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  type="button"
                  onClick={closeModal}
                  className="rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-white/75"
                >
                  Cancel
                </button>
              </div>
